...
* [`src/device_locking/`](src/device_locking/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/device_locking/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `device-locking` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/device_locking/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns device-locking.example
  (:require [device-locking.config :as config])
